...
def remote_detect(stream=None, options=None, api=None, args=None):
    ...
    g.logger.Debug(2,
                   'Invoking mlapi with url:{} and json: mid={} reason={} stream={}, stream_options={} ml_overrides={} headers={} params={} '.format(
                       object_url, mid, reason, stream, options, ml_overrides, auth_header, params))
    ...
    if g.config['write_image_to_zm'] == 'yes' and matched_data['frame_id']:
        ...
        try:
            ...
            if matched_data['image_dimensions'] and matched_data['image_dimensions']['resized']:
                ...
                if oldw != neww:
                    g.logger.Debug(2,
                                   'Resizing source image from width={} to width={} in zm_detect as that is the size mlapi used'.format(
                                       oldw, neww))
                    img = imutils.resize(img, width=neww)
                else:
                    g.logger.Debug(2, 'No need to resize as image widths are same from mlapi and zm_detect: {}'.format(
                        neww))
...
def main_handler():
    ...
    try:
        ...
        g.logger.Fatal(
            '{}: You might not have installed OpenCV as per install instructions. Remember, it is NOT automatically '
            'installed'.format(e))

    g.logger.Debug(1, '---------| app:{}, pyzm:{}, ES:{} , OpenCV:{}|------------'.format(__app_version__, pyzm_version,
                                                                                          es_version, cv2.__version__))
    ...
    if pred != '':
        ...
        if (matched_data['image'] is not None) and (
                g.config['write_image_to_zm'] == 'yes' or g.config['write_debug_image'] == 'yes'):
            debug_image = pyzmutils.draw_bbox(image=matched_data['image'], boxes=matched_data['boxes'],
                                              ...
                                              polygons=matched_data['polygons'],
            ...
            if g.config['write_debug_image'] == 'yes':
                for _b in matched_data['error_boxes']:
                    cv2.rectangle(debug_image, (_b[0], _b[1]), (_b[2], _b[3]),
                                  (0, 0, 255), 1)
                filename_debug = g.config['image_path'] + '/' + os.path.basename(
                    append_suffix(stream, '-{}-debug'.format(matched_data['frame_id'])))
        ...
        if g.config['create_animation'] == 'yes':
            if not args.get('eventid'):
                ...
                try:
                    img.createAnimation(matched_data['frame_id'], args.get('eventid'),
                                        args.get('eventpath') + '/objdetect', g.config['animation_types'])
                except Exception as e:
